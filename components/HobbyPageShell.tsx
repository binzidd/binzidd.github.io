...
export const PROJECT_META: Record<string, ProjectMeta> = {
  ...
  budget: {
    ...
    note: "All Sankey node positions computed analytically from column assignments and vertical flow balancing — no library layout engine. Values cross-validated against MYEFO final estimates.",
  },
  ...
  f1: {
    ...
    findings: [
      ...
      "Ferrari's upgrade at Monaco delivered genuine Q3 competitiveness — their wind tunnel correlation issues from 2024 partially resolved by mid-season.",
    ...
    <div className="min-h-screen" style={{ background: "#000500" }}>
      ...
      <nav className="sticky top-0 z-40 flex items-center justify-between px-5 py-3"
        ...
        <Link href="/#hobbies"
          ...
          style={{ color: "#006600", fontFamily: "var(--font-mono),monospace", textDecoration: "none" }}
          onMouseEnter={(e) => { (e.currentTarget as HTMLAnchorElement).style.color = "#00FF41"; }}
          onMouseLeave={(e) => { (e.currentTarget as HTMLAnchorElement).style.color = "#006600"; }}>
          <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
            <path d="M19 12H5M12 5l-7 7 7 7" />
          </svg>
          binay.is-a.dev
        </Link>
        <span className="text-[9px] hidden sm:block" style={{ color: "#003300", fontFamily: "var(--font-mono),monospace" }}>
          {p.code}
        </span>
        <a href={p.githubUrl} target="_blank" rel="noopener noreferrer"
          className="flex items-center gap-1.5 px-3 py-1.5 rounded-full text-[10px] transition-all duration-150"
          style={{ color: "#00FF41", border: "1px solid #003300", fontFamily: "var(--font-mono),monospace", textDecoration: "none" }}
          onMouseEnter={(e) => { (e.currentTarget as HTMLAnchorElement).style.borderColor = "#00FF41"; }}
          onMouseLeave={(e) => { (e.currentTarget as HTMLAnchorElement).style.borderColor = "#003300"; }}>
      ...
      <header className="max-w-5xl mx-auto px-6 pt-16 pb-10">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
          ...
          <p className="text-[9px] tracking-[0.3em] uppercase mb-3"
            style={{ color: "#006600", fontFamily: "var(--font-mono),monospace" }}>{p.code}</p>
          ...
          <div className="flex flex-wrap gap-2">
            {p.tags.map((t) => (
              <span key={t} className="text-[10px] px-2.5 py-0.5 rounded-full"
                style={{ background: "#020c02", color: "#006600", border: "1px solid #003300", fontFamily: "var(--font-mono),monospace" }}>
                {t}
              </span>
            ))}
          </div>
      ...
      <motion.div className="max-w-5xl mx-auto px-6 mb-10"
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 rounded-2xl overflow-hidden divide-x divide-y md:divide-y-0 divide-[#003300]"
          style={{ border: "1px solid #003300", background: "#020c02" }}>
          {p.metrics.map((m, i) => (
            <div key={i} className="px-5 py-4 text-center">
              <p className="text-xl md:text-2xl font-light tabular-nums mb-0.5"
                style={{ color: "#00FF41", fontFamily: "var(--font-mono),monospace" }}>{m.value}</p>
              <p className="text-[9px] uppercase tracking-widest"
                style={{ color: "#8B949E", fontFamily: "var(--font-inter),sans-serif" }}>{m.label}</p>
              {m.sub && (
      ...
      <div className="max-w-5xl mx-auto px-6 pb-20">
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
              ...
              {p.findings.map((f, i) => (
                <li key={i} className="flex gap-3">
                  <span className="text-[10px] mt-0.5 flex-shrink-0 tabular-nums"
                    style={{ color: "#003300", fontFamily: "var(--font-mono),monospace" }}>
          ...
          <motion.div className="lg:col-span-2 flex flex-col gap-4"
            ...
            {/* Data sources */}
            <div className="rounded-2xl p-5" style={{ background: "#020c02", border: "1px solid #003300" }}>
              ...
              <div className="flex flex-wrap gap-1.5">
                {p.stack.map((s) => (
                  <span key={s} className="text-[9px] px-2 py-0.5 rounded"
                    style={{ background: "#000500", color: "#006600", border: "1px solid #003300", fontFamily: "var(--font-mono),monospace" }}>
                    {s}
                  </span>
                ))}
              </div>
