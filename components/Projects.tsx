...
const statusConfig: Record<Project["status"], { label: string; color: string; bg: string }> = {
  ...
  "pre-prod": { label: "Pre-Production", color: "#00FF41", bg: "rgba(0,255,65,0.1)"   },
  ...
  alumni:     { label: "Alumni",         color: "#006600", bg: "rgba(0,102,0,0.2)"    },
};
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
      ...
      <motion.div initial={{ opacity: 0, scale: 0.95, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: 20 }}
        ...
        style={{ background: "#020c02", border: "1px solid #003300", boxShadow: "0 0 60px rgba(0,255,65,0.05)" }}
        onClick={(e) => e.stopPropagation()}>
        <div className="px-8 pt-8 pb-6" style={{ background: "#020c02", borderBottom: "1px solid #003300" }}>
          <div className="flex items-start justify-between gap-4">
            ...
            <button onClick={onClose} className="flex-shrink-0 w-8 h-8 rounded-full flex items-center justify-center transition-colors" style={{ background: "#003300" }}
              onMouseEnter={(e) => e.currentTarget.style.background = "#006600"} onMouseLeave={(e) => e.currentTarget.style.background = "#003300"}>
        ...
        <div className="px-8 py-6">
          {project.highlight && (
            <div className="flex items-center gap-2 mb-5 px-4 py-3 rounded-xl" style={{ background: "rgba(0,255,65,0.06)", border: "1px solid rgba(0,255,65,0.15)" }}>
              <span className="text-base">✨</span>
              <span className="text-xs font-medium" style={{ color: "#00FF41", fontFamily: "var(--font-mono), monospace" }}>{project.highlight}</span>
            </div>
          )}
          <p className="text-sm leading-relaxed mb-6" style={{ color: "#8B949E", fontFamily: "var(--font-inter), sans-serif" }}>{project.description}</p>
          <p className="text-[10px] font-semibold tracking-[0.15em] uppercase mb-3" style={{ color: "#006600", fontFamily: "var(--font-mono), monospace" }}>// tech_stack</p>
          <div className="flex flex-wrap gap-2">
            {project.tech.map((t) => (
              <span key={t} className="px-3 py-1 rounded-full text-[11px] font-medium" style={{ background: "#020c02", color: "#8B949E", border: "1px solid #003300", fontFamily: "var(--font-mono), monospace" }}>{t}</span>
            ))}
          </div>
        </div>
      </motion.div>
