      ...
      <div className="max-w-3xl mx-auto">
                      ...
                      <div className="px-6 py-5">
                        <div className="flex items-start justify-between gap-4">
                          ...
                          <motion.div animate={{ rotate: isOpen ? 45 : 0 }} transition={{ duration: 0.25 }}
                            ...
                            style={{ background: isOpen ? "rgba(0,217,255,0.1)" : "transparent" }}>
                            <svg width="12" height="12" viewBox="0 0 12 12" fill="none"><path d="M6 1v10M1 6h10" stroke={isOpen ? "#00D9FF" : "#484F58"} strokeWidth="1.5" strokeLinecap="round" /></svg>
                          </motion.div>
                        </div>
                      </div>
                          ...
                          <motion.div key="content" initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.4, ease: EASE }} style={{ overflow: "hidden" }}>
                            <div className="px-6 pb-6 space-y-5">
                              ...
                              {exp.sections.map((section, si) => (
                                <div key={si}>
                                  <p className="text-[10px] font-semibold tracking-[0.15em] uppercase mb-3" style={{ color: "#00D9FF", fontFamily: "var(--font-mono), monospace" }}>{section.heading}</p>
                                  <ul className="space-y-2">
                                    {section.bullets.map((bullet, bi) => (
                                      <motion.li key={bi} custom={si * 10 + bi} variants={bulletVariants} initial="hidden" animate="visible"
                                        ...
                                        <span className="mt-2 flex-shrink-0 w-1 h-1 rounded-full" style={{ background: "#00D9FF44" }} />
                                        {bullet}
                                      </motion.li>
                                    ))}
                                  </ul>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.3, duration: 0.6 }} className="mt-14 flex justify-center">
          <button onClick={() => setChatOpen(true)}
            ...
            style={{ background: "#111720", border: "1px solid #21262D", fontFamily: "var(--font-mono), monospace" }}
            onMouseEnter={(e) => { e.currentTarget.style.borderColor = "rgba(0,217,255,0.3)"; e.currentTarget.style.boxShadow = "0 0 20px rgba(0,217,255,0.06)"; }}
            ...
            <span className="text-[10px] px-2 py-0.5 rounded-full" style={{ background: "rgba(0,217,255,0.08)", color: "#00D9FF", border: "1px solid rgba(0,217,255,0.15)" }}>⌘K</span>
          </button>
        </motion.div>
      </div>
      ...
      {chatOpen && (
        <div className="fixed inset-0 z-50 flex items-end md:items-center justify-center p-4" style={{ background: "rgba(0,0,0,0.7)" }} onClick={() => setChatOpen(false)}>
          <motion.div initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 40 }} onClick={(e) => e.stopPropagation()}
            className="w-full max-w-lg rounded-2xl p-6" style={{ background: "#111720", border: "1px solid #21262D" }}>
            <p className="text-xs mb-2" style={{ color: "#484F58", fontFamily: "var(--font-mono), monospace" }}>// Press ⌘K for the full terminal Q&A interface</p>
            <div className="mt-4 space-y-2">
              {["Tell me about Adobe", "What is Project SPUR?", "What GenAI projects has he built?"].map((q) => (
                <button key={q} onClick={() => { setChatOpen(false); setTimeout(() => { document.dispatchEvent(new CustomEvent("palette-query", { detail: q })); }, 200); }}
                  ...
                  style={{ background: "#161C26", color: "#8B949E", fontFamily: "var(--font-mono), monospace", border: "1px solid #21262D" }}
                  onMouseEnter={(e) => e.currentTarget.style.borderColor = "rgba(0,217,255,0.2)"}
                  onMouseLeave={(e) => e.currentTarget.style.borderColor = "#21262D"}>
                  &gt; &ldquo;{q}&rdquo;
                </button>
              ))}
            </div>
            <button onClick={() => setChatOpen(false)} className="mt-4 w-full py-2 text-xs rounded-full" style={{ color: "#484F58", fontFamily: "var(--font-mono), monospace" }}>// close</button>
          </motion.div>
        </div>
      )}
    </section>
