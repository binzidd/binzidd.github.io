...
export default function Hero() {
  ...
  return (
    <section id="hero" className="relative min-h-screen flex flex-col items-center justify-center overflow-hidden dot-grid"
      ...
      {/* Glow orbs */}
      <div className="orb-1 absolute rounded-full pointer-events-none" style={{ width: 600, height: 600, top: "-150px", right: "-100px", background: "radial-gradient(circle, rgba(0,217,255,0.06) 0%, transparent 70%)", filter: "blur(60px)" }} />
      ...
      <div className="absolute inset-0 pointer-events-none overflow-hidden">
        <motion.div animate={{ y: ["0%", "100vh"] }} transition={{ duration: 10, repeat: Infinity, ease: "linear" }}
          className="absolute left-0 right-0 h-px" style={{ background: "linear-gradient(90deg, transparent, rgba(0,217,255,0.04), transparent)" }} />
      </div>
      ...
      <div className="relative z-10 text-center px-6 max-w-4xl mx-auto">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2, duration: 0.7 }}
          ...
          <span className="h-px w-10" style={{ background: "linear-gradient(90deg, transparent, #00D9FF)" }} />
          <span className="text-[10px] tracking-[0.3em] uppercase" style={{ color: "#00D9FF", fontFamily: "var(--font-mono), monospace" }}>
            Sydney, Australia // Data &amp; GenAI
          </span>
          <span className="h-px w-10" style={{ background: "linear-gradient(90deg, #00D9FF, transparent)" }} />
        </motion.div>
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 1.0, duration: 0.5 }}
          ...
          <span className="text-base md:text-xl" style={{ color: "#8B949E", fontFamily: "var(--font-mono), monospace" }}>
            ...
            <span style={{ color: "#00D9FF" }}>{displayed}</span>
            <span className="cursor-blink inline-block w-[2px] h-5 ml-0.5 align-middle" style={{ background: "#00D9FF" }} />
          </span>
        </motion.div>
        ...
        <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 1.1, duration: 0.7 }}
          ...
          {stats.map((stat, i) => (
            <motion.div key={stat.label} initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }}
              ...
              <span className="text-3xl md:text-4xl font-semibold" style={{ color: "#00D9FF", fontFamily: "var(--font-cormorant), serif" }}>
                {stat.value}<span className="text-2xl">{stat.suffix}</span>
              </span>
              <span className="text-[10px] mt-1 text-center" style={{ color: "#484F58", fontFamily: "var(--font-mono), monospace", letterSpacing: "0.08em" }}>
                {stat.label}
              </span>
            </motion.div>
          ))}
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 1.5, duration: 0.6 }}
          ...
          <button onClick={() => document.getElementById("timeline")?.scrollIntoView({ behavior: "smooth" })}
            ...
            style={{ background: "#00D9FF", color: "#0A0E14", fontFamily: "var(--font-mono), monospace" }}
            onMouseEnter={(e) => { e.currentTarget.style.boxShadow = "0 0 24px rgba(0,217,255,0.4)"; e.currentTarget.style.transform = "translateY(-2px)"; }}
            onMouseLeave={(e) => { e.currentTarget.style.boxShadow = "none"; e.currentTarget.style.transform = "translateY(0)"; }}>
            ./explore_work
          </button>
          <a href="https://linkedin.com/in/binaysiddharth" target="_blank" rel="noopener noreferrer"
            className="px-7 py-3 rounded-full text-sm font-medium transition-all duration-300"
            style={{ border: "1px solid #21262D", color: "#8B949E", fontFamily: "var(--font-mono), monospace" }}
            onMouseEnter={(e) => { e.currentTarget.style.borderColor = "rgba(0,217,255,0.3)"; e.currentTarget.style.color = "#00D9FF"; e.currentTarget.style.transform = "translateY(-2px)"; }}
            onMouseLeave={(e) => { e.currentTarget.style.borderColor = "#21262D"; e.currentTarget.style.color = "#8B949E"; e.currentTarget.style.transform = "translateY(0)"; }}>
            connect --linkedin
          </a>
        </motion.div>
      </div>
      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 2.0, duration: 0.8 }}
        ...
        <span className="text-[10px] tracking-[0.2em] uppercase" style={{ color: "#484F58", fontFamily: "var(--font-mono), monospace" }}>scroll</span>
        <motion.div animate={{ y: [0, 8, 0] }} transition={{ duration: 1.8, repeat: Infinity, ease: "easeInOut" }}
          className="w-px h-10" style={{ background: "linear-gradient(to bottom, #00D9FF44, transparent)" }} />
      </motion.div>
    </section>
  );
}
